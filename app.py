import os, time, requests, threading, logging, json
    ...
    try:
        if os.path.exists(USERS_FILE):
            with open(USERS_FILE, "r") as f:
                data = json.load(f)
                log.info(f"Loaded {len(data)} users from disk.")
                return data
    except: pass
    return {}
...
def do_check():
    ...
    try:
        ...
        state["status"]    = "ok"
        log.info(f"Check #{state['checks']} — {len(seats)} seats — {len(users)} users")
        for seat in seats:
...
log.info(f"=== {len(users)} users loaded from disk ===")
